 ...
 * @author YanLu
 */
public class SelectMovieSeatActivity extends Activity implements OnTouchListener,View.OnLongClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        columView = (LinearLayout) findViewById(R.id.seatcolumn);
        button_click = (Button) findViewById(R.id.button_click);

        button_click.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                //do something
                        Toast.makeText(SelectMovieSeatActivity.this,"hello",Toast.LENGTH_SHORT).show();
                    }
              });
        //设置透明度
        alpha = new AlphaAnimation(0.6F, 0.6F);
        alpha.setDuration(10000); // Make animation instant
        ...
        seatTableView.setSeatTable(seatTable);//状态信息
        seatTableView.setRowSize(maxRow);//最大行
        seatTableView.setColumnSize(maxColumn);//最大列
        seatTableView.setOnTouchListener(this);//触摸监听
        seatTableView.setOnLongClickListener(this);
        seatTableView.setLinePaint(paint);//中线
    ...
    public boolean onTouch(View v, MotionEvent event) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (!eatClick && i != -1 && j != -1 && i == oldClick[0] && j == oldClick[1] ) {
                    ...
                    if (selectedSeats.isEmpty())
                        button_click.setVisibility(View.GONE);
                    }else{

                    button_click.setVisibility(View.VISIBLE);
                }
                break;
        }
        ...
        if (!eatClick || diffY > 5 || diffX > 5 || diffScal > 0.01) {//减少绘制次数
           ...
           // mFocusY = minTop > 0 ? Math.max(-minTop, Math.min(mFocusY, defWidth * mScaleFactor))
            //        : 0;
        ...
        Log.v(TAG,"点击触摸范围"+Boolean.toString(isClickSeat));
    ...
    public boolean  onLongClick(View view){
        Toast.makeText(SelectMovieSeatActivity.this,"diji触发",Toast.LENGTH_SHORT).show();
        Log.v(TAG,"长按触发");
        return true;
    }
    //左侧的座位列号
    public void onChanged() {
        ...
        rowView.setBackgroundColor(getResources().getColor(R.color.trans_black));
        ...
        for (int i = 0; i < seatTableView.getRowSize(); i++) {
            ...
            for (int j = 0; j < seatTableView.getColumnSize(); j++) {
                if (seatTable[i][j] != null) {
                    textView.setText(Integer.toString(seatTable[i][j].rowName));
                    break;
                }
            }
        ...
        columView.removeAllViews();
        columView.setBackgroundColor(getResources().getColor(R.color.trans_black));
        columView.startAnimation(alpha);

        columView.setPadding((int) (mFocusX),
                getResources().getDimensionPixelSize(R.dimen.padding_1dp), 0, 0);//左右移动

        for (int k = 0; k < seatTableView.getColumnSize(); k++) {
            TextView textView = new TextView(SelectMovieSeatActivity.this);
            //座位有可能为空
            for (int h = 0; h < seatTableView.getRowSize(); h++) {
                if (seatTable[h][k] != null) {
                    textView.setText(Integer.toString(seatTable[h][k].colName));
                    break;
                }
            }
            ...
            textView.setGravity(Gravity.CENTER);
            textView.setLayoutParams(new ViewGroup.LayoutParams(
                    (int)(defWidth * mScaleFactor),LinearLayout.LayoutParams.WRAP_CONTENT ));
            textView.setPadding( 0,getResources().getDimensionPixelSize(R.dimen.padding_2dp),
                    0,getResources().getDimensionPixelSize(R.dimen.padding_2dp) );
            columView.addView(textView);
        }
